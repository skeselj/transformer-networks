...
class G(nn.Module):
    def __init__(self, k=5, f=nn.ReLU()):
        ...
        p = (k-1)/2
        self.conv1 = nn.Conv2d( 2, 48, k, padding=p)
        self.conv2 = nn.Conv2d(48, 48, k, padding=p)
        self.conv3 = nn.Conv2d(48, 48, k, padding=p)
        self.conv4 = nn.Conv2d(48, 48, k, padding=p)
        self.conv5 = nn.Conv2d(48,  2, k, padding=p)
        ...
        #        raise ValueError("There shouldn't be BatchNorm yet")
...
class Pyramid(nn.Module):
    ...
    def forward(self, stack, idx, lastlevel):   # stack: B x 2 x _ x _
        ...
        field_so_far, residuals_so_far = self.forward(self.down(stack), idx+1, lastlevel) # B, _, _, 2
        ...
        # excluded level: pass it on
            ...
            residual = Variable(torch.zeros(field_so_far.size()), requires_grad=False).cuda()
...
class PyramidTransformer(nn.Module):
    ...
    def forward(self, x, lastlevel=0):
